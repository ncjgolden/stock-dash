...
메인 **오늘의 투자판단** 화면은 제공된 최종 디자인을 기준으로 **다크 투자 코크핏** 형태로 정리했습니다.

**시장 상황 → 종목 현재 위치 → 핵심 투자지표 → 가격·기술적 분석 → 투자판단 레이더 → 조건별 투자전략 → 실적·기업분석·수급 → 업종별 투자지도**

화면의 핵심 원칙은 다음과 같습니다.

- 상단에는 KOSPI·KOSDAQ·S&P500·SOX·원/달러의 시장 맥락을 배치합니다. 실제 시장 데이터가 연결되기 전에는 값을 임의 생성하지 않고 **시장 데이터 연결 필요**로 표시합니다.
- 종목 영역에는 종목명·코드·업종·현재가·일간 등락과 52주 위치·시장 상태·거래대금/거래량 영역을 배치합니다. 연결되지 않은 값은 미확인 상태로 남깁니다.
- 핵심 투자지표에는 매출 성장률·영업이익 성장률·영업이익률·외국인·기관·적정가 참고값을 배치하고, 기간과 계산 기준을 함께 표시합니다.
- 기술적 분석은 **일봉·완료 주봉·추세·RSI·MACD·20MA**를 확장할 수 있는 자리로 고정하며, 실제 가격 시계열이 있을 때만 차트를 표시합니다.
- 투자판단은 점수 자체보다 **펀더멘털·수급·밸류에이션·모멘텀·리스크의 확인 근거**를 보여줍니다.
- 투자전략은 상승/중립/하락의 확률을 임의로 제시하지 않고, 각 조건이 충족될 때 **무엇을 다시 확인할지**를 보여주는 조건형 시나리오로 구성합니다.
- 실적·기업분석에서는 매출·영업이익·주력사업·경쟁사 영업이익을 함께 확인하고, 시장 수급에서는 외국인·기관과 추가 시장 데이터 연결 영역을 구분합니다.
- 화면 최하단에는 **업종별 투자지도**를 유지해 개별 종목에서 시장/업종으로 시야를 확장합니다. 실제 업종별 등락률·거래대금·수급은 검증 가능한 `sector.performance` provider가 연결된 경우에만 표시합니다.
- 하단에는 데이터 기준일·출처·계산 기준과 미확인 항목을 표시해 숫자의 출처와 한계를 확인할 수 있게 합니다.

기존의 DART·공공데이터 기반 종목 분석과 채팅 조사 결과를 같은 화면의 대시보드 데이터로 통합합니다. 저장된 공식 결산 분석도 대시보드에서 이어서 확인할 수 있으며, 데이터가 없는 항목은 **데이터 연결 필요/미확인**으로 표시합니다.

기존 종목 분석·공시·투자일지·계좌 연결·교육자료·데이터 연결 관리 기능은 유지합니다. 대시보드는 기존 기능을 대체하지 않고 그 위에 투자판단용 요약 화면을 추가하는 구조입니다.

최종 디자인 반영 커밋:
- `9b0e69415d56eb856681c7b662f1f3c6beecfdf6` · 기존 대시보드 UI
- `c6eea65648f2148da0c3871bff8f0735a3139870` · 기존 저장 분석과 대시보드 통합
- `7280b7e28cb8429e70b9100ccaffe65596b2cb5f` · 최종 다크 투자 코크핏 디자인
- `953187d8e1799615c3613c301dd76e9e44d80c49` · 최종 화면 포맷 보강
